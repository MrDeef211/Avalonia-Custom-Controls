...
---
# DataFormControl
Автоматически строит форму редактирования для любого объекта на основе рефлексии. Поддерживает группировку по категориям, вложенные строки, валидацию и команды.
...
- `FormConfig` – Конфигурационный дбьект формы (`DataFormConfig`)
...
| `IsReadOnly`	| `boo`l	| `false`	| `Глобальный запрет редактирования всех полей.` | 
...
| `GridSizeX`	| `GridLength`	| `Auto`	| Шаг сетки по `X`. `Auto` – автоматический подбор; число – фиксированный шаг; `N*` – ровно `N` линий. | 
...
Интерактивность можно активировать только при зажатой клавише-модификаторе, задав свойство `InteractiveModifier` (например, `Control`). Если модификатор не требуется, установите `InteractiveModifier="None"`.
...
| `ZoomOutKey`	| `Subtract` (и всегда `OemMinu`s) | 
...
| Свойство	| Тип	| По умолчанию	| Описание | 
| :--- | :--- | :--- | :--- |
| `TickStep`	| `GridLength`	| `Auto`	| Шаг засечек. Auto — автоматический подбор красивого шага; N* — ровно N засечек на всём диапазоне; число — фиксированный шаг в единицах Minimum–Maximum. | 
...
Не перегружайте тулбар — если элементов много, рассмотрите возможность сворачивания части в выпадающее меню.
