...
| `RestrictPan`	| `bool`	| `true`	| Ограничивать панорамирование (См. ниже). | 
| `EnableKeyboardNavigation`	| `bool`	| `true`	| Включить управление с клавиатуры. | 

`RestrictPan` свойство ограничивающее панорамирование в области контента. Для содержимого ZoomControl вычисляется доступная для перемещения область, равная области видной при максимальном отдалении, это также значит, что при максимальном отдалении нельзя сдвинуться.
...
- **Toolbar** — плоский контейнер для кнопок и разделителей с автоматическими отступами и стилями.
- **ToolbarGroup** — группирует горизонтально несколько элементов (кнопки, переключатели) и отображает под ними текстовую подпись.
